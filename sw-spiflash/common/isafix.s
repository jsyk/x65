...
; ISAFIX_ENTRY:           ; entry point for ISAFIX handler at address $07e000,
    ...
    lda     f:NORA_RMBCTRL_REG
    and     #$3F
    sta     f:NORA_RMBCTRL_REG
...
.A8
    ...
    ; Map BOOTROM back to the memory space (because we are returning there via RTL below)
    ; by setting bits 7 and 6 of the NORA_RMBCTRL_REG register.
    lda     f:NORA_RMBCTRL_REG
    ora     #NORA_RMBCTRL__MAP_BOOTROM | NORA_RMBCTRL__AUTO_UNMAP_BOOTROM
                ; NORA_RMBCTRL__AUTO_UNMAP_BOOTROM will cause the BOOTROM to be unmapped on the next RTI,
                ; which is executted by the PBL EMUABORT handler.
    sta     f:NORA_RMBCTRL_REG

    ; now we can go back to the abort handler in PBL to finish off.
    RTL

; constant data, part of the function
maskbits:
    .byte $01
    .byte $02
    .byte $04
